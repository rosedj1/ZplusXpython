...
NOTE:   Before running script, do:
        ...
        If the edmPickEvents.py part searches through data sets VERY quickly,
        this probably indicates that the command is failing.
        To determine any errors, do this in the shell:
        `edmPickEvents.py "/SingleMuon/Run2017B-UL2017_MiniAODv2-v1/MINIAOD" \
        297178:716:891914663`
        You'll likely need to point your certificate to the right spot.
        This may help:
        `export X509_USER_PROXY=/path/to/myproxy`

        Having trouble running this file on HPG. Run on lxplus instead.
...
from Utils_Python.Utils_Files import (
    open_json, check_overwrite, replace_value, make_dirs
    )
...
input_template = "/cmsuf/data/store/user/t2/users/rosedj1/sampleprod/UL/CMSSW_10_6_26/src/UFHZZAnalysisRun2/UFHZZ4LAna/python/templateData_UL17_10626_2l_cfg_template.py"
outdir_root = "/cmsuf/data/store/user/t2/users/rosedj1/ZplusXpython/sidequests/rootfiles/syncwithfilippo/"
outdir_txt = "/cmsuf/data/store/user/t2/users/rosedj1/ZplusXpython/sidequests/txt/"
...
def run_bbfana_single_evt(
    ...
    """
    template_dir = os.path.dirname(ana_template)
    outdir_template = os.path.join(template_dir, "template_copies")
    make_dirs(outdir_template, outdir_root, outdir_txt)
